...
export const KanbanDataContextProvider = ({ children }) => {
    ...
    const deleteCategory = async (categoryId, setTodoCategories) => {
        try {
            const response = await axios.delete('/kanban', { data: { id: categoryId } });
            setTodoCategories(response.data);
            setError(null);
        } catch (error) {
            handleError(error.message);
    ...
    const clearAllTasks = async (categoryId) => {
        try {
            const response = await axios.delete('/kanban/clearTasks', { data: { categoryId } });
    ...
    const addCategory = async (categoryName) => {
        try {
            const response = await axios.post('/kanban/addCategory', { categoryName });
    ...
    const deleteTodo = async (taskId, setTodoCategories) => {
        try {
            const response = await axios.delete(`/kanban/${taskId}`);
            setTodoCategories(response.data);
