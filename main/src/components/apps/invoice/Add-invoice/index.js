...
const CreateInvoice = () => {
  ...
  useEffect(() => {
    if (invoices.length > 0) {
      const lastId = parseInt(invoices[invoices.length - 1].id);
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.billFromEmail && !emailRegex.test(formData.billFromEmail)) {
      errors.billFromEmail = 'Invalid email format';
    }
    if (formData.billToEmail && !emailRegex.test(formData.billToEmail)) {
      errors.billToEmail = 'Invalid email format';
    }
    
    // Validate phone numbers if provided
    const phoneRegex = /^[+]?[(]?[0-9]{1,4}[)]?[-\s.]?[0-9]{1,4}[-\s.]?[0-9]{1,4}[-\s.]?[0-9]{1,4}$/;
    if (formData.billFromPhone && !phoneRegex.test(formData.billFromPhone)) {
      errors.billFromPhone = 'Invalid phone number';
    }
    if (formData.billToPhone && !phoneRegex.test(formData.billToPhone)) {
      errors.billToPhone = 'Invalid phone number';
    }
    
    // Validate addresses
    if (!formData.billFromAddress) errors.billFromAddress = 'From Address is required';
    if (!formData.billToAddress) errors.billToAddress = 'To Address is required';
    
    // If there are validation errors, don't submit
    if (Object.keys(errors).length > 0) {
      console.error('Validation errors:', errors);
      // You could set these errors to state and display them in the UI
      return;
    }
